...
function connect() {
    ...
    if (url[1] === 'mynetwork') {
        if (url.length > 2 && url[2].toLowerCase() === 'invite-connect') {
            ...
            // Soft filter workflow
            mode = 'soft';
            if (settings !== null) {
                dayCycle(Object.assign({}, default_settings, settings), mode);
                // dayCycle(deepmerge(default_settings, settings));     // neat option, but concatenates arrays
            } else {
                dayCycle(default_settings, mode);
            }
        }
    } else if (url[1] === 'search' && url[2] === 'results') {
        // Hard filter workflow
        mode = 'hard';
        console.log('Hard filter workflow');
        if (settings !== null) {
            dayCycle(Object.assign({}, default_settings, settings), mode);
        } else {
            dayCycle(default_settings, mode);
